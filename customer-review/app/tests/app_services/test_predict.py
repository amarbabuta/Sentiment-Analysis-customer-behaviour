...
DATA_FOLDER_PATH = "app/tests/app_services/data/predict/test_predict_00.json"
...
def test_predict():
    ...
    # launch prediction_job
    endpoint_path = "model_predict"
    
    logging.info(f"check endpoint {endpoint_path} - start") 
    logging.info("form_data_to_post_for_predict_from_dictionary")
    ...
    logging.info(f"Sentiment: {response.content_as_json['sentiment']}")
